...
static void ssl_write_trusted_ca_key_ext( mbedtls_ssl_context *ssl,
										  unsigned char *buf,
										  size_t *olen )
{
	unsigned char *p = buf;
	...
	unsigned char trusted_authority[23];
	size_t trusted_ca_key_len = sizeof( trusted_authority );
	struct mbedtls_x509_crt *nextCrt;
	uint8_t num_of_trusted_authorities = 0;

	*olen = 0;

	MBEDTLS_SSL_DEBUG_MSG( 3, ( "client hello, adding trusted ca key extension") );
	...
	trusted_authority[0] = (unsigned char)( ( (sizeof(trusted_authority) - 2) >> 8 ) & 0xFF );
	trusted_authority[1] = (unsigned char)( ( (sizeof(trusted_authority) - 2)      ) & 0xFF );
	trusted_authority[2] = MBEDTLS_SSL_CA_ID_TYPE_CERT_SHA1_HASH;

	*p++ = (unsigned char)( ( MBEDTLS_TLS_EXT_TRUSTED_CA_KEY >> 8 ) & 0xFF );
	*p++ = (unsigned char)( ( MBEDTLS_TLS_EXT_TRUSTED_CA_KEY      ) & 0xFF );

	/* length will be written after counting of the authorities
	 */
	p++;
	p++;

	nextCrt = ssl->conf->ca_chain;

	while(nextCrt != NULL) {

		mbedtls_sha1(nextCrt->raw.p, nextCrt->raw.len, &trusted_authority[3]); // Get SHA1Hash

		if( end < p || (size_t)( end - p ) < trusted_ca_key_len)
		{
			MBEDTLS_SSL_DEBUG_MSG( 1, ( "buffer too small" ) );
			break;
		}

		memcpy( p, trusted_authority, trusted_ca_key_len );
		p += trusted_ca_key_len;
		num_of_trusted_authorities++;
		nextCrt = nextCrt->next;
	}

	*length++ = (unsigned char)( ( ( trusted_ca_key_len * num_of_trusted_authorities ) >> 8 ) & 0xFF );
	*length++ = (unsigned char)( ( ( trusted_ca_key_len * num_of_trusted_authorities )      ) & 0xFF );

	*olen = (trusted_ca_key_len * num_of_trusted_authorities) + 4;
...
static int ssl_write_client_hello( mbedtls_ssl_context *ssl )
{
	...
	*q++ = (unsigned char)( n << 1 );
	...
	//
#if defined(MBEDTLS_SSL_TRUSTED_CA_KEY)
	ssl_write_trusted_ca_key_ext( ssl, p + 2 + ext_len, &olen);
	ext_len += olen;
#endif
