                                          ...
                                          size_t *olen )
{
	...
	unsigned char trusted_auth[23];
	size_t trusted_ca_key_len = sizeof( trusted_auth );
    struct mbedtls_x509_crt *next_crt;
	uint8_t num_of_trusted_auth = 0;

    *olen = 0;

    MBEDTLS_SSL_DEBUG_MSG( 3, ( "client hello, adding trusted_ca_key extension" ) );
	...
	trusted_auth[0] = (unsigned char)( ( ( sizeof(trusted_auth) - 2 ) >> 8 ) & 0xFF );
	trusted_auth[1] = (unsigned char)( ( ( sizeof(trusted_auth) - 2 )      ) & 0xFF );
	trusted_auth[2] = MBEDTLS_SSL_CA_ID_TYPE_CERT_SHA1_HASH;
    ...
    *p++ = (unsigned char)( ( MBEDTLS_TLS_EXT_TRUSTED_CA_KEY      ) & 0xFF );
    ...
    next_crt = ssl->conf->ca_chain;

    while( next_crt != NULL )
    {
		mbedtls_sha1( next_crt->raw.p, next_crt->raw.len, &trusted_auth[3] ); // Get SHA1Hash
		...
		memcpy( p, trusted_auth, trusted_ca_key_len );
        p += trusted_ca_key_len;
		num_of_trusted_auth++;
        next_crt = next_crt->next;
    }

	*len++ = (unsigned char)( ( ( trusted_ca_key_len * num_of_trusted_auth ) >> 8 ) & 0xFF );
	*len = (unsigned char)( ( ( trusted_ca_key_len * num_of_trusted_auth )      ) & 0xFF );

	*olen = ( trusted_ca_key_len * num_of_trusted_auth ) + 4;
}
